...
namespace EdFi.Ods.Security.Authorization.Repositories
{
        ...
        where TEntity : class
    {
        ...
        public ICriteria GetCriteriaQuery(TEntity specification, IQueryParameters queryParameters)
        {
            ...
            // var allFilterDetails = authorizationFilters.Select(
            //     x => new
            //     {
            //         ClaimEndpointName = x.Value.Keys.First(),
            //         SubjectEndpointName = x.Key.Split(new[] {"To"}, StringSplitOptions.None)
            //             .FirstOrDefault(y => y != x.Value.FirstOrDefault().Key)
            //             ?? x.Value.FirstOrDefault().Key,
            //         ViewName = x.Key,
            //         ClaimValues = x.Value
            //     })
            //     .ToArray();
